...
/*Endika 29/07/2020*/
    /*Changeset 816 en TFS DigitSeguros*/
/*Comentado para solucionar problema Scroll que no llega hasta el final*/
...
function errorValidationMsg() {
    Swal.fire({
        type: 'error',
        title: 'Faltan campos obligatorios',
        text: 'Por favor, rellene todos los campos obligatorios antes de guardar.'
    })
}

function checkIfFormIsValid() {
    if (!$("form").valid()) {
        showNumberOfValidationErrors(countValidationErrors());
    }
}
function countValidationErrors() {
    var elementList = document.getElementsByClassName("error");
    var generalDataList = ["businessAddress", "delegationList", "actividad_generica", "Plan_GeneralData_PlanTitle", "cliente", "Plan_GeneralData_CreatorName", "cargo", "tecnico"];
    var additionalContentList = ["empresa_estudio", "promotor", "localizacion", "municipality", "Plan_AdditionalData_OrganizationalStructure", "Plan_AdditionalData_ActivityDescription", "Plan_AdditionalData_AssistanceCenters"]
    var generalDatataElementList = [];
    var additionalContentElementList = [];

    for (let i = 0; i < elementList.length; i++) {
        var element = elementList[i];
        var contGeneralData = 0;
        var contadditionalContent = 0;
        
        if (element.innerHTML.includes("This field is required") || element.innerHTML.includes("Este campo es obligatorio")) {
            for (let j = 0; j < generalDataList.length; j++) {
                if (element.id.includes(generalDataList[j]))
                    generalDatataElementList.push(element);
                else if (element.id.includes(additionalContentList[j]))
                    additionalContentElementList.push(element);
            }      
        }
    }
    var errors = new Object();
    errors.generalData = generalDatataElementList.length;
    errors.additionalContent = additionalContentElementList.length;

    return errors;
}

function showNumberOfValidationErrors(errors) {
    var generalDataErrorCounterElement = document.getElementById("generalDataErrorMsg");
    var additionalContentErrorCounterElement = document.getElementById("additionalContentErrorMsg");
    var pestanaAditionalContent = document.getElementById("pestanaAdditionalContent");
    var pestanaGeneralData = document.getElementById("pestanaGeneralData");

    document.body.scroll(0, 0);
    if (errors.generalData > 0 || errors.additionalContent > 0) {
        errorValidationMsg();  
    }

    if (errors.generalData > 0) {
        generalDataErrorCounterElement.textContent = errors.generalData;
        pestanaGeneralData.style.marginRight = "0px !important";
        generalDataErrorCounterElement.style.display = "";
    }

    else {
        generalDataErrorCounterElement.style.display = "none"
        pestanaGeneralData.style.marginRight = "20px !important";
    }

    if (errors.additionalContent > 0) {
        additionalContentErrorCounterElement.textContent = errors.additionalContent;
        pestanaAditionalContent.style.marginRight = "0px !important";
        additionalContentErrorCounterElement.style.display = "";
    }

    else {
        additionalContentErrorCounterElement.style.display = "none"
        pestanaAditionalContent.style.marginRight = "20px !important";
    }
}
